	...
	function addCombo( editor, comboName, styleType, lang, entries, defaultLabel, styleDefinition, order ) {
		...
		editor.ui.addRichCombo( comboName, {
			...
			onClick: function( value ) {
				...
				} else if ( !isRemove && ( !hasAppliedNewStyle( range ) || range.collapsed ) ) {
				...
				function hasStyleToRemove( range ) {
					...
					while ( textNode ) {
						if ( textNode.getAscendant( hasSearchedStyle ) ) {
							return true;
						}
				...
				function hasAppliedNewStyle( range ) {
					var walker = new CKEDITOR.dom.walker( range ),
						textNode;
					...
					while ( textNode ) {
						var ascendantNode = textNode.getAscendant( hasSearchedStyle );
				...
				function hasSearchedStyle( el ) {
					return el.type === CKEDITOR.NODE_ELEMENT && el.getStyle( searchedStyle )
				}
